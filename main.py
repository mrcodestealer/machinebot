...
def _handle_machine_message(
    ...
    if (cmd in ("/set", "/setmachine")
        ...
        try:
            ...
            send_message(
                chat_id,
                json.dumps(_setnow_mod.build_form_card(_sn_sid, _sn_sess)),
                msg_type="interactive",
            )
...
def lark_webhook():
    ...
    if card_resolved is not None:
        ...
        if isinstance(parsed_sync, dict):
            ...
            # /set buttons update the card IN-PLACE — same 3s window as /sst, and the machines
            # box is a form so its text arrives as form_value exactly like /sst's.
            try:
                import setnow as _setnow_sync

                if str(parsed_sync.get("k") or "").strip().lower() == _setnow_sync.SETNOW_CARD_KEY:
                    ...
                    _gt_resp = _setnow_sync.handle_card_callback(
                        parsed_sync,
                        chat_id=chat_id_ca or "",
                        send_card=_sst_send_card,
                        run_batch=_sst_run_batch,
                ...
                print(f"❌ set card callback failed: {_gt_sync_err!r}", flush=True)
